...
function changeFieldRequestHandler(event) {
    ...
    if (excursionDate.value != '' && startTime.value != '') {
        ...
        if (option2.checked) {
            let temp = calculateTotalCost;
            calculateTotalCost *= isThisDayOff ? 1.25 : 1.30;
...
async function controlItemsHandler(event) {
    ...
    if (action) {
        ...
        if (action == 'show') {
            ...
            nameOption2.innerHTML = 'Сопровождение сурдопереводчика';
            descOption1.innerHTML = 'Стоимость уменьшается на';
            descOption2.innerHTML = 'Сопровождение сурдопереводчика увеличивает стоимость на (группы 1-5 человек/5-10 человек)';
            discountOption1.value = '15%';
            discountOption2.value = '15%/25%';
...
async function buttonChooseGuideHandler(event) {
    ...
    option2Desc.innerHTML = 'Сопровождение сурдопереводчика увеличивает стоимость на (группы 1-5 человек/5-10 человек)';
