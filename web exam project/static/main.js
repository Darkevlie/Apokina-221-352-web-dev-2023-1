...
// инициализация карты 2 гис
const map = new mapgl.Map('map-2-gis', {
    key: API_KEY_2GIS,
    // координаты центра Москвы 37.619742, 55.754462
    center: [37.619742, 55.754462],
    zoom: 10,
});
// глобальная переменная для начальной позиции на карте
let marker0;
// глобальная переменная для конечной позиции на карте
let marker1;
...
function changeFieldRequestHandler(event) {
    ...
    let buttonCreateRequest = document.querySelector("#createRequest button");
    ...
    if (excursionDate.value != '' && startTime.value != '') {
        ...
        let hasSignLanguageInterpreter = option2.checked;
        let calculateTotalCost = calculateCost(priceGuide.value,
            duration.value, isThisDayOff, isItMorning,
            isItEvening, numberOfPeople.value, hasSignLanguageInterpreter);

        let option1amount = formInputs['discount-amount-1'];
        let option2amount = formInputs['discount-amount-2'];

        // Блокировка кнопки при выборе более 10 человек
        if (numberOfPeople.value > 10) {
            buttonCreateRequest.disabled = true;
            option2.disabled = true;
        } else {
            buttonCreateRequest.disabled = false;
        ...
        if (option2.checked) {
            let temp = calculateTotalCost;
            calculateTotalCost *= numberOfPeople>5 ? 1.25 : 1.15;
...
/**
 * Обработчик события для поля количества посетителей
 */
function numberOfPeopleChangeHandler() {
    let numberOfPeopleInput = document.querySelector("#createRequest [name='number-of-people']");
    let option2 = document.querySelector("#createRequest [name='option-2']");
    if (numberOfPeopleInput.value > 10) {
        option2.disabled = true;
...
// Добавляем обработчик события для поля количества посетителей
document.querySelector("#createRequest [name='number-of-people']").addEventListener('change', numberOfPeopleChangeHandler);
...
async function buttonChooseGuideHandler(event) {
    ...
    option2Name.innerHTML = 'сопровождение сурдопереводчика';
    option2Desc.innerHTML = 'Стоимость увеличивается на';
    option2amount.value = '15%/25%';
...
async function buttonChooseRouteHandler(event) {
    ...
    let dataRoute = await dataExchangeWithTheServer('get', 'route',
        {}, idRoute);
    marker1 = new mapgl.Marker(map, {
        coordinates: dataRoute['coords'][0],
        label: {
...
async function convertAddressInCoordinates(qParam) {
    ...
    if (data['meta']['code'] == 200) {
        ...
        // В СЛУЧАЕ ОШИБКИ, ЧТОБЫ ПРОВЕРИТЬ РАБОТОСПОСОБНОСТЬ 
        // ОСТАВШИХСЯ ФУНКЦИЙ БЕРУТСЯ ЗАДАННЫЕ МНОЙ ЗНАЧЕНИЯ
        longitude = 55.784369;
        latitude = 37.711060;
    }
    let coordinates = [longitude, latitude];
    marker0 = new mapgl.Marker(map, {
